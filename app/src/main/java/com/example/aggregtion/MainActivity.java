...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //mTextView = findViewById(R.id.tet_view);
        ...
        findViewById(R.id.create_folder_btn).setOnClickListener(view->createFolder());
       // findViewById(R.id.open_a_file_btn).setOnClickListener(view -> Cre);

        // Authenticate the user. For most apps, this should be done when the user performs an
        // action that requires Drive access rather than in onCreate.
        requestSignIn();
        //Task<String> fileId = mDriveServiceHelper.createFile();
        //mDriveServiceHelper.saveFile(fileId.getResult(),"Test","Hello World");
        //Task<FileList> fileListTask = mDriveServiceHelper.queryFiles();
        //FileList fileList = fileListTask.getResult();
        //List<File> filel = fileList.getFiles();
        //for(int i =0 ;i<filel.size();i++)
        //{
        //   mTextView.setText(filel.get(i).getName());
        //}
       /* Task<FileList> fileListTask = mDriveServiceHelper.queryFiles();
        FileList fileList = fileListTask.getResult();
        List<File> filel = fileList.getFiles();
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    private void openFileFromFilePicker(Uri uri) throws FileNotFoundException {
        if (mDriveServiceHelper != null) {
            Log.d(TAG, "Opening " + uri.getPath());

            String id = DocumentsContract.getDocumentId(uri);
            InputStream inputStream = getContentResolver().openInputStream(uri);
            java.io.File file1 = new java.io.File(getCacheDir().getAbsolutePath()+"/"+id);
            writeFile(inputStream, file1);
            String filePath1 = file1.getAbsolutePath();


            mDriveServiceHelper.openFileUsingStorageAccessFramework(getContentResolver(), uri,filePath1)
        ...
        if (mDriveServiceHelper != null) {
            ...
            mDriveServiceHelper.queryFiles()
                    .addOnSuccessListener(fileList -> {
                        ...
                        //Task<FileList> fileListTask = mDriveServiceHelper.queryFiles();
                        //FileList fileList2 = fileListTask.getResult();
                        List<File> filel = fileList.getFiles();
                        getSupportFragmentManager().beginTransaction().replace(
                                R.id.frameLayoutActivityMain,
                                new QueryFragment(filel)
                        ).commit();
    ...
    void writeFile(InputStream in, java.io.File file) {
